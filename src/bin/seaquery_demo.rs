//! Adapted from https://github.com/SeaQL/sea-query/tree/master/examples/sqlx_postgres.
...
#[tokio::main]
async fn main() {
    let pool = PgPool::connect("postgres://testuser:testpassword@127.0.0.1:9999/testdb")
        .await
        .unwrap();
    ...
    let result = sqlx::query(&sql).execute(&mut *pool_conn).await;
